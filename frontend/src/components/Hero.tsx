...
const AGENTS = [
  { name: 'security_drone', icon: '🐝', label: 'Security', color: 'from-red-500/20 to-orange-500/20', border: 'border-red-500/30' },
  { name: 'logic_wasp', icon: '🐝', label: 'Logic', color: 'from-cyan-500/20 to-blue-500/20', border: 'border-cyan-500/30' },
  { name: 'style_bee', icon: '🐝', label: 'Style', color: 'from-amber-500/20 to-yellow-500/20', border: 'border-amber-500/30' },
  { name: 'cultural_drone', icon: '🐝', label: 'Culture', color: 'from-purple-500/20 to-pink-500/20', border: 'border-purple-500/30' },
  { name: 'growth_queen', icon: '🐝', label: 'Growth', color: 'from-emerald-500/20 to-teal-500/20', border: 'border-emerald-500/30' },
  { name: 'senior_architect', icon: '🐝', label: 'Architect', color: 'from-indigo-500/20 to-blue-500/20', border: 'border-indigo-500/30' },
]
...
export function Hero({ onSwarm, onFileUpload, code, language, targetLanguage, setCode, setLanguage, setTargetLanguage, isSwarming, status, error, findings, voicePrompt, setVoicePrompt, setVoiceLanguage }: HeroProps) {
  ...
  const handleSwarmClick = () => {
    ...
    onSwarm(code, language, targetLanguage)
  }

  return (
    <section className="relative z-10 max-w-7xl mx-auto px-6 pt-10 pb-4">
      <FlyingBees trigger={beeTrigger} />
            ...
            <div className="mt-4">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className={`absolute bottom-3 right-3 px-3 py-1.5 rounded-full transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${
                    isRecording
                  ...
                  title={isRecording ? 'Stop recording' : isTranscribing ? 'Transcribing...' : 'Record voice context'}
            ...
            <div className="h-[22rem] overflow-y-auto space-y-2 pr-1 rounded-xl bg-bespoke-bg p-4 border border-bespoke-border shadow-inner">
              {findings && findings.length > 0 ? (
                <div className="space-y-2">
                  {findings.map((f: any, i: number) => {
                    ...
                    return (
                      ...
                      >
                        <div className="mt-1 text-bespoke-accent">{agent?.icon || '🐝'}</div>
                        <div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-bespoke-muted animate-pulse">Bees reviewing your code...</p>
                </div>
              ) : status === 'failed' || error ? (
                <div className="text-center py-12 flex flex-col items-center">
                  <p className="text-sm font-medium text-red-400">Swarm Analysis Failed</p>
                  <p className="text-xs mt-1 text-red-400/80 max-w-xs">{error || 'The swarm failed to process this code.'}</p>
                </div>
              ) : (
